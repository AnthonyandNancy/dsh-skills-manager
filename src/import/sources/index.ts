...
function projectRoots(cwd: string | undefined, relative: string): string[] {
  return cwd === undefined || cwd.length === 0 ? [] : [join(cwd, relative)]
}

export const claudeSource: ExternalSkillSource = {
  id: 'claude',
  label: 'Claude Code',
  getSkillRoots(cwd) {
    return [
...
export const codexSource: ExternalSkillSource = {
  id: 'codex',
  label: 'OpenAI Codex',
  getSkillRoots(cwd) {
    return [
...
export const cursorSource: ExternalSkillSource = {
  id: 'cursor',
  label: 'Cursor',
  getSkillRoots(cwd) {
    return [
...
export const geminiSource: ExternalSkillSource = {
  id: 'gemini',
  label: 'Gemini CLI',
  getSkillRoots(cwd) {
    return [
...
export const externalSources: readonly ExternalSkillSource[] = [
  claudeSource,
  codexSource,
  cursorSource,
  geminiSource,
...
export function sourceLabel(id: ExternalSourceId): string {

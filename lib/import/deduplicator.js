 ...
 */
function item(candidate, result, reason, importedSkillId, extra = {}) {
    return {
...
export function deduplicateCandidates(candidates, existing) {
    ...
    for (const snap of existing) {
        ...
        if (snap.fingerprint !== undefined)
            existingByFingerprint.set(snap.fingerprint, snap);
    }
    ...
    const representativeGroupKey = new Map();
    let alreadyInDsh = 0;
...
export function buildDuplicateGroups(items) {
    ...
    for (const item of items) {
        ...
        if (item.result === 'duplicate') {
            ...
            if (item.duplicateReason === 'already-in-dsh')
                group.inDsh = true;
        }

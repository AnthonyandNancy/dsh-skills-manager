...
import { withSkillsManagerView, withViewScope } from "./skills-view.js";
import { resolveDshSkillsRoot } from "./import/importer.js";
import { serializeSkillFile } from "./import/parser.js";
/** Kebab-case skill name grammar, matching DSH's native Skill name rule. */
const SKILL_NAME = /^[a-z0-9]+(?:-[a-z0-9]+)*$/;
function isSkillName(name) {
    return SKILL_NAME.test(name);
}
async function nativeSkills(ctx) {
    return await createSkillAccess(ctx.ctx);
}
/** Resolve the DSH-managed roots that this plugin is allowed to write/delete. */
export function managedSkillRoots(ctx, cwd) {
    const roots = [
...
export function assertManagedSkillPath(path, roots) {
...
export async function listManagedSkills(ctx, cwd) {
    return await withSkillsManagerView(ctx.ctx, async (view) => {
        const summaries = await view.registry.list(withViewScope(view, { cwd }));
        ctx.ctx.logger?.info?.('[dsh-skills-manager] skills.list: cwd=%s scopeSource=%s registry=host count=%d', cwd ?? '-', view.scopeSource, summaries.length);
        const rows = [];
...
export async function getManagedSkill(ctx, name, cwd) {
...
export async function createManagedSkill(ctx, input) {
    const name = input.name.trim();
    if (!isSkillName(name))
        throw new Error(`invalid kebab-case skill name "${name}"`);
...
export async function updateManagedSkill(ctx, input, cwd) {
    const skills = await nativeSkills(ctx);
    const definition = await skills.get(input.name, { cwd });
    if (definition === undefined)
        throw new Error(`skill "${input.name}" not found`);
    if (definition.path === undefined)
        throw new Error(`skill "${input.name}" is not filesystem-backed and cannot be edited`);
...
export async function deleteManagedSkill(ctx, name, cwd) {
    const skills = await nativeSkills(ctx);
    const definition = await skills.get(name, { cwd });
    if (definition === undefined)
        throw new Error(`skill "${name}" not found`);
    if (definition.path === undefined)
        throw new Error(`skill "${name}" is not filesystem-backed and cannot be deleted`);
    const roots = managedSkillRoots(ctx, cwd);
    assertManagedSkillPath(definition.path, roots);
...
export async function readSkillFile(path) {

...
function leaseRelease(lease) {
    ...
    if (typeof dispose !== 'function')
        return undefined;
    return async () => { await dispose.call(lease); };
}
function warnUnusableScope(ctx, error) {
    ctx.logger?.warn?.('[dsh-skills-manager] default agent preset scope unavailable; listing the global skill layer instead: %s', messageOf(error));
}
/**
 ...
 */
export async function resolveSkillsManagerView(ctx) {
 ...
 */
export async function withSkillsManagerView(ctx, read) {
    ...
    try {
        ...
        if (view.release !== undefined) {
            try {
                ...
                ctx.logger?.warn?.('[dsh-skills-manager] failed to release the default agent preset scope lease: %s', messageOf(error));
            }
        }
    }
}
/** Apply the resolved view's scope to one registry lookup. */
export function withViewScope(view, options = {}) {
    return {
        ...options,
